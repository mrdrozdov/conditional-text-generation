...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512):
        ...
        if False and os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            s_length = collections.Counter()

            with open(file_path, encoding="utf-8") as f:
                for line in f:
                    prefix, sentence, unused2, unused3 = line.split('_')
                    prefix = prefix.strip()
                    sentence = sentence.strip()
                    my_length = len(sentence.split())
                    s_length[my_length] += 1
                    ...
                    # TODO: Add option so that there is no split token.
                    tok_prefix = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(prefix))
                    ...
                    for i, w in enumerate(s.split()):
                        if i > 0:
                            w = ' ' + w
                        raw_tok = tokenizer.tokenize(w)
                        tok = tokenizer.convert_tokens_to_ids(raw_tok)
                        if i == 0:
                            tok_first = tok
                        else:
                            tok_rest += tok
                    ...
                    if args.prefix:
                        tokenized_text = tok_prefix + [SPLIT_ID] + tok_first + tok_rest
                        mask = [TI_PAD] * len(tok_prefix) + [TI_PAD] + [TI_FIRST_TOK] * len(tok_first) + [TI_TOK] * len(tok_rest)
                    else:
                        tokenized_text = [SPLIT_ID] + tok_first + tok_rest
                        mask = [TI_PAD] + [TI_FIRST_TOK] * len(tok_first) + [TI_TOK] * len(tok_rest)
                    #tokenized_text = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(prefix)) + [SPLIT_ID] + tokenizer.convert_tokens_to_ids(tokenizer.tokenize(sentence))
                    ...
                    if len(tokenized_text) < block_size:
                        # pad right
                        mask = mask + [TI_PAD] * (block_size - len(tokenized_text))
                        tokenized_text = tokenized_text + [PADDING_ID] * (block_size - len(tokenized_text))

                    assert len(mask) == len(tokenized_text)
            ...
            #sofar = 0
            #for k in sorted(s_length.keys()):
            #    sofar += s_length[k]
            #    logger.info("{} {}".format(k, sofar))
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, (batch, batch_mask) in enumerate(epoch_iterator):
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch, batch_mask in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            _labels = shift_labels[shift_mask]
            _logits = shift_logits[shift_mask]

            loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
            loss = loss_fct(_logits, _labels)
            lm_loss = loss

            num_bytes = _labels.shape[0]
            cache['num_bytes'].append(num_bytes)
            cache['loss'].append(lm_loss.item())
            # END

            EXTRA = any([args.show_predictions, args.write_xent])

            if EXTRA:

                if args.write_xent:
                    batch_size = shift_labels.shape[0]
                    first_tok_mask = batch_mask[:, :].contiguous() == 1
                    rest_tok_mask = batch_mask[:, :].contiguous() == 2
                    tok_mask = torch.logical_or(first_tok_mask, rest_tok_mask)

                    for i_batch in range(batch_size):
                        _ids = labels[i_batch][tok_mask[i_batch]] # ID is not a TOK
                        _toks = tokenizer.convert_ids_to_tokens(_ids)
                        _probs = shift_prob[i_batch][shift_mask[i_batch]]

                        n = _ids.shape[0]
                        n_first = n - _probs.shape[0]

                        local_cache = collections.defaultdict(list)

                        for i_tok in range(n):
                            _id = _ids[i_tok].item()
                            tok = _toks[i_tok]
                            gold = tokenizer.convert_tokens_to_string(tok)
                            gold_prob = -1
                            if i_tok >= n_first:
                                gold_prob = _probs[i_tok - n_first, _id].item()
                            gold_rank = -1
                            local_cache['gold'].append(gold)
                            local_cache['gold_prob'].append(gold_prob)
                            local_cache['gold_rank'].append(gold_rank)
    ...
    lst = []
    for x, y in zip(cache['loss'], cache['num_bytes']):
        lst += [x] * y
